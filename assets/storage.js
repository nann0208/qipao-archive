...
const ALL_TOPICS = [
  ...
  '国货展览会', '外贸', '服制条例', '服装制作工艺', '民族国家', '更名争议'
];
...
// 舆论类型（仅适用于报刊文章）
const OPINION_TYPES = ['规范约束型', '体验反馈型', '价值重构型'];
const OPINION_TYPE_COLORS = {
  '规范约束型': '#0495C8',
  '体验反馈型': '#C370CE',
  '价值重构型': '#E74583'
};
...
// 按舆论类型筛选（仅报刊文章有此字段，支持多选）
function filterByOpinionType(records, opinionTypes) {
  if (!opinionTypes || opinionTypes.length === 0) return records;
  // 如果传入单个字符串（向后兼容），改成数组
  const filters = Array.isArray(opinionTypes) ? opinionTypes : [opinionTypes];
  return records.filter(r => {
    const types = Array.isArray(r.opinion_types) ? r.opinion_types : (r.opinion_types ? [r.opinion_types] : []);
    // 记录的舆论类型与任一筛选条件有重合即可
    return filters.some(f => types.includes(f));
  });
}

// 工具函数：获取舆论类型对应颜色
function getOpinionTypeColor(opinionType) {
  return OPINION_TYPE_COLORS[opinionType] || '#888';
}
